...
3. **Forensic Accountability**: AI actions taken in response to errors MUST reference specific log entries found during Forensic Analysis. No "blind troubleshooting."
4.  **Static Validation**: Any code block generated for an engine MUST be validated via **Pyright** (Blender) or **Roslyn/ApiMetadataMapper** (Unity) before commit.
5.  **Strict Serializability**: The Orchestrator enforces a total order of intents. Operation B is only processed after Operation A is finalized (Committed or Rolled Back). No interleaving of mutations is permitted.
5.  **Monotonic Causality**: All sub-operations MUST carry the `ParentID` of their originating transaction and a unique `MonotonicID` generated by the Go Kernel. Monotonic counters are the sole authority for ordering.
