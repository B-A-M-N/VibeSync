...
3.  **Monotonic Causality**: All sub-operations MUST carry the `ParentID` of their originating transaction and a unique `MonotonicID` generated by the Go Kernel.
...
3.  **Whitelisted APIs**: Only use approved Go SDK, Blender (`bpy`), and Unity (`UnityEditor`) calls.
4.  **Single API Layer**: All mutations MUST go through the `vibe-mcp-server`. No direct direct access to engine internals from raw AI scripts.
5.  **Numerical Safety**: All transforms and material deltas must pass the `auditPayload` check for NaN/Inf.
